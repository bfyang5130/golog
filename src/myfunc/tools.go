...
func UtcTimeToNormalDateTime(timeString string) (dateTime string, err error) {
	tString := strings.SplitN(timeString, ` `, -1)
	if t1, err := time.Parse("02/Jan/2006:15:04:05", strings.TrimSpace(tString[0])); err == nil {
		return t1.Format(`2006-01-02 15:04:05`), nil
	} else {
...
func Convertip_CachTiny(ip string, buf []byte) (ipCountry, ipProvince, ipCity string) {
	//----------
	toIp := net.ParseIP(ip).To4()
	if toIp == nil {
		return ``, ``, ``
	}
	//获得IP的标记地址
	...
	if index_length > 0 {
		ipCountry, ipProvince, ipCity = SplitToCPC(string(buf[readindex : readindex+uint32(index_length)]))
	}
...
func Convertip_tiny(ip string) (ipCountry, ipProvince, ipCity string) {
	...
	//fmt.Println(ipdot[0])
	//读取IP二进制文件
	//fi, err := os.Open("/var/golang/src/logFit/source/tinyipdata.dat")
	fi, err := os.Open("../logFit/source/tinyipdata.dat")
...
func ReadIpDatToBuf(fileIpPath string) (buf []byte, err error) {
